...
impl CostModelParams {
    ...
    pub fn for_scale(
        ...
        let (teardowns, provisions, tenants, tenant_hours, sustained_hours,
             training_hours, jobs, oversub_hours, ghost_lifetime) = match scale {
    ...
    pub fn project(&self) -> CostProjection {
        ...
        let w_ghost = self.ghost_rate * self.avg_ghost_frac
            * self.teardowns_per_day * 365.0
            * self.ghost_lifetime_hours * self.gpu_hourly_rate;

        // W_contention = contention_drop * tenant_hours * 365 * rate * (1 - 1/tenants)
        let w_contention = (self.contention_drop_pct / 100.0)
            * self.active_tenant_hours_per_day * 365.0
        ...
        let w_straggler = (self.straggler_tax_pct / 100.0)
            * self.training_job_hours * self.jobs_per_day * 365.0
            * fleet_factor * self.gpu_hourly_rate;

        // W_oversub = waste_pct * oversub_hours * 365 * rate
        let w_oversub = (self.oversub_waste_pct / 100.0)
            * self.oversubscribed_hours_per_day * 365.0
            * self.gpu_hourly_rate;

        let total_per_gpu = w_ghost + w_contention + w_provisioning
            + w_burst_gap + w_straggler + w_oversub;
...
mod tests {
    ...
    fn test_fleet_scaling() {
        let small = CostModelParams::for_scale(FleetScale::Small, 0.03, 0.003, 15.0, 0.3, 20.0, 5.0, 8.0).project();
        let medium = CostModelParams::for_scale(FleetScale::Medium, 0.03, 0.003, 15.0, 0.3, 20.0, 5.0, 8.0).project();
        let large = CostModelParams::for_scale(FleetScale::Large, 0.03, 0.003, 15.0, 0.3, 20.0, 5.0, 8.0).project();

        assert!(large.total_fleet > medium.total_fleet, "large fleet > medium");
        assert!(medium.total_fleet > small.total_fleet, "medium fleet > small");
    }
}
